...
async function main() {
    ...
    // ---------------- Promotions ----------------
    const promosData = [
        { code: "BIENVENIDO10", description: "10% de descuento en tu primera compra", discountType: "PERCENTAGE", discountValue: 10, validUntil: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000), usageLimit: 500 },
        { code: "ENVIOGRATIS", description: "Envío gratis en pedidos delivery", discountType: "FIXED", discountValue: 5, validUntil: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), usageLimit: 300 },
        { code: "COMBO15", description: "15% de descuento en combos seleccionados", discountType: "PERCENTAGE", discountValue: 15, validUntil: new Date(Date.now() + 45 * 24 * 60 * 60 * 1000), usageLimit: 200 },
    ];
    ...
    const statuses = ["DELIVERED", "DELIVERED", "DELIVERED", "PENDING", "PREPARING", "ON_THE_WAY", "DELIVERED", "CANCELLED", "DELIVERED", "REFUNDED"];
    const types = ["DELIVERY", "PICKUP", "DINE_IN", "DRIVE_THRU", "DELIVERY", "DELIVERY", "PICKUP", "DELIVERY", "DRIVE_THRU", "DELIVERY"];
    ...
    if (existingOrders === 0) {
        for (let i = 0; i < 10; i++) {
            ...
            const items = chosen.map((p) => ({ productId: p.id, name: p.name, price: p.price, quantity: 1 + Math.floor(Math.random() * 2) }));
            ...
            await prisma.order.create({
                data: {
                    ...
                    status: statuses[i],
                    type: types[i],
                    address: types[i] === "DELIVERY" ? { street: "Av. España 1234", city: "Trujillo" } : client_1.Prisma.JsonNull,
